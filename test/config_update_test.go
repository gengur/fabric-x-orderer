...
func TestCreateConfigBlockNUpdate(t *testing.T) {
	...
	// sharedConfig, _, err := config.LoadSharedConfig(configPath)
	...
	//data["data"].(map[string]any)["data"].([]any)[0].(map[string]any)["payload"].(map[string]any)["data"].(map[string]any)["config"]
	...
	err = os.WriteFile(jsonConfigPath, jsonConfigData, 0644)
	require.NoError(t, err)

	// map[string]any)["channel_group"].(map[string]any)["groups"].(map[string]any)["Orderer"].(map[string]any)["values"].(map[string]any)["BatchSize"].(map[string]any)["value"].(map[string]any)["max_message_count"]
	...
	err = os.WriteFile(modifiedJsonConfigPath, modifiedJsonConfigData, 0644)
	require.NoError(t, err)
